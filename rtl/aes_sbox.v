...
//
// Description:
//   Combinational AES S-box (SubBytes substitution table) per FIPS 197,
//   Section 5.1.1. Implements the affine transformation over GF(2^8)
//   as a 256-entry lookup table. Synthesizes to distributed LUTs on
//   Xilinx Artix-7 (~64 LUT6s per instance).
//
// Interface:
//   in  [7:0]  — Input byte to substitute
//   out [7:0]  — Substituted output byte
//
...
module aes_sbox (
    ...
    // FIPS 197 Table: S-box substitution values
    // Organized as sbox[row][col] where row = in[7:4], col = in[3:0]
    always @(*) begin
        case (in)
            // Row 0x00
